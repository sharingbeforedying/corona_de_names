...
class Game {
  ...
  constructor(teamsConfigWrapper, nb_cells = 25) {
    // this.config = config;
  ...
  configureLinkFrom(gameModel) {
    ...
    if(gameModel.addBonusTurnEventListener) {
      gameModel.addBonusTurnEventListener(turn => {
        ...
        const gameTurn = new GameTurn(index, teamId, tellerId, guesserId, 1);
         ...
         gameTurn.canEndTurn = true; // this is ugly

         //this is also ugly
         {
           const PlayerAction = require("../PlayerAction.js").PlayerAction;
           const TellerHint   = require("../actions/TellerHint.js").TellerHint;

           // const srcId  = turn.tellerId;
           // const hint   = turn.hint;
           // const previousGameTurn = this.gameTurns[this.gameTurns.length -1];
           const previousGameTurn = this.gameState.turn;
           const srcId = previousGameTurn.tellerId;
           const hint  = previousGameTurn.tellerStep.playerAction.action;
           console.log("previousTellerStep", previousGameTurn.tellerStep);

           const action = new TellerHint(hint.word, hint.number);

           const playerAction = new PlayerAction(srcId, action);
           const result       = true;

           const instanceStateTurn = gameTurn;
         ...
         this.rx_bonusTurn.next(gameTurn);
      });
    }
    ...
    gameModel.addTellerPhaseEventListener(turn => {
...

      {
        ...
        if(playerId == teller_red.id) {
          outString = "TEAM1 (TELLER)";
        } else if(playerId == teller_blue.id) {
          outString = "TEAM2 (TELLER)";
        } else {
          outString = "???";
        }
    ...
    gameModel.addGuesserPhaseEventListener(turn => {
...

      {
        ...
        if(playerId == guesser_red.id) {
          outString = "TEAM1 (GUESSER)";
        } else if(playerId == guesser_blue.id) {
          outString = "TEAM2 (GUESSER)";
        } else {
          outString = "???";
        }
  ...
  manageTellerAction(playerAction) {
    ...
    //validate
    if(!this.validateTeller(playerAction, instanceStateTurn)) {
      return;
    }
  ...
  manageGuesserSelection(playerAction) {
    ...
    //validate
    if(!this.validateGuesser(playerAction, instanceStateTurn)) {
      return;
    }
  ...
  validateTeller(playerAction, instanceStateTurn) {
    var outBool = false;

    if(instanceStateTurn.tellerStep != null) {
      //ignore
      console.log("this is not teller phase => ignore");
    } else {
      const playerId = playerAction.playerId;
      if(playerId != instanceStateTurn.tellerId) {
        //ignore
        console.log("this is not the expected teller => ignore");
      } else {
        outBool = true;
      }
    }
  ...
  validateGuesser(playerAction, instanceStateTurn) {
    var outBool = false;

    if(instanceStateTurn.tellerStep == null) {
      //ignore
      console.log("this is not guesser phase => ignore");
    } else {
      const playerId = playerAction.playerId;
      if(playerId != instanceStateTurn.guesserId) {
        //ignore
        console.log("this is not the expected guesser => ignore");
      } else {
        outBool = true;
      }
    }
  ...
  */
  ...
  initializeGameState(gameState) {
    ...
    gameState.position_grid__blank = this.positionGridForCnpGridModel(this.gameModel.gridModel, 2);
  ...
  positionGridForCnpGridModel(cnpGridModel, teamIndex) {
    ...
    switch(teamIndex) {
      ...
      case 1:
        ...
        break;
      case 2:
        cellModels = cnpGridModel.cellModels_red; //it doesn't matter (ugliness is my co-pilot)
        break;
      default:
