...
const Enums = require('../enums_instance.js');
...
class GameModel {
  ...
  manageCellSelection(cellIndex, player, team, selectionCallback) {
    ...
    this.sendDidUpdateCellModelEvent(gameCellModel);
...
class ContractItem {
  ...
  validateAction(act, on_valid, on_invalid, on_gameover) {
    console.log("ContractItem", "validateAction", act);

    const cellModel = act.cellModel;
    const actTeam   = act.team;

    if(actTeam != this.team) {
