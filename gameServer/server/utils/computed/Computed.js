...
class FieldPath {
  constructor(path) {
    this.path      = path;
    this.pathArray = FieldPath.pathArray(path);
    this.source    = this.pathArray[0];
  }
...
class FieldFormula {
  ...
  constructor(fieldName, triggerFieldPathsArray, computeFunc, initialFunc = null) {
    this.fieldName         = fieldName;

    this.triggerFieldPathsArray = triggerFieldPathsArray;
    this.targetSources          = triggerFieldPathsArray.map(fieldPath => fieldPath.source);
