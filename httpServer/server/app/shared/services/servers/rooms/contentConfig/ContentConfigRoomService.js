...
export class ContentConfigRoomService extends AbsRoomService {
  ...
  constructor(room) {
    ...
    room.onMessage("answer", (message) => {
      console.log("~moveToNextRoom listener, message received from server");
      console.log(message);
    ...
    const rx_contentConfig = new ReplaySubject(1);

    const contentConfig = room.state.contentConfig;
    console.log("initial contentConfig", contentConfig);
    rx_contentConfig.next(contentConfig);  //initial value
    ...
    room.state.onChange = (changes) => {
      changes.forEach(change => {
          ...
          if(change.field == "contentConfig") {
            const contentConfig = room.state.contentConfig;
            // this.observed_messages.notifyObservers(change.value);
            // this.observed_messages.notifyObservers(messageArray);
            rx_contentConfig.next(contentConfig);
          }

      });
    };
    this.rx_contentConfig = rx_contentConfig.asObservable();


    const rx_cell_change =  new Rx.Subject();
    room.state.contentConfig.grid.cells.onChange = (value, key) => {
      console.log("room.state.contentConfig.grid.cells.onChange", "changes", value, key);
